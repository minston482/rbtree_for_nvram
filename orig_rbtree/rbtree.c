...
unsigned int simul_rb_insert (simul_rb_root *T, long long int inkey) {
	unsigned int ret = 0;
...
unsigned int simul_rb_delete (simul_rb_root *T, long long int delkey) {
	unsigned int ret = 0;
...
int main(int argc, char *argv[])
{
	...
	for( i = 0; i< 10; i++) {
		...
		if (node == root->nil_node)
			printf("%lld is not in rbtree\n", i);
		else
			printf("%lld is in rbtree\n",i);
	}
	printf("end of search\n");
	printf("pre-order start\n");
	...
	printf("after del 8 pre-order start\n");
	simul_rb_pre_order(root, root->root_node->rb_left);
	printf("pre-order end\n");
	...
	printf("after del 2 pre-order start\n");
	simul_rb_pre_order(root, root->root_node->rb_left);
	printf("pre-order end\n");
	...
	printf("after del 3 pre-order start\n");
	simul_rb_pre_order(root, root->root_node->rb_left);
	printf("pre-order end\n");
