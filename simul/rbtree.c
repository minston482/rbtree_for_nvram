...
int simul_rb_delete(simul_rb_root *T, long long int delkey) {
	...
	if (y != del_node) {
		...
		printf("key %lld color %s\t\n", del_node->rb_key, del_node->rb_color == SIMUL_RB_RED ? "red" : "black");
		free(del_node);
	}
	else {
		if (is_rb_black(y))
			simul_rb_delete_fixup(T, x);
		printf("key %lld color %s\t\n", y->rb_key, y->rb_color == SIMUL_RB_RED ? "red" : "black");
		free(y);
	}
...
void simul_rb_pre_order(simul_rb_root *T, simul_rb_node *node) {
	printf("key %lld color %s\t\n", node->rb_key, node->rb_color == SIMUL_RB_RED ? "red" : "black");
